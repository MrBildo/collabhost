# Collabhost — Collaboard

Collabhost development is tracked on its Collaboard instance.

**Board slug:** `collabhost`
...
### Titles
Action-oriented for features (e.g., "Add process supervisor service"). Bug-report style for bugs. Keep under 80 characters.
...
**Card addressing:** Use `cardNumber` + `boardSlug` (e.g., card #3 on `collabhost`)
**Auth key:** stored in `.agents.env` (gitignored). Use THIS project's key for the collabhost board.
**Board slug:** `collabhost`
