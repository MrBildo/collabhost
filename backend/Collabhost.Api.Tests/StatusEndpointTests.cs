...
public class StatusEndpointTests(CollabhostApiFixture fixture) : IClassFixture<CollabhostApiFixture>
{
    ...
    public async Task GetStatus_ReturnsOkWithExpectedShape()
    {
        ...
        root.GetProperty("toolsDirectory").GetString().ShouldNotBeNullOrWhiteSpace();
    }

    [Fact]
    public async Task GetStatus_ToolsDirectory_IsAbsolutePath()
    {
        // Arrange
        var client = _fixture.CreateClient();
        ...
        var content = await response.Content.ReadAsStringAsync();
        var json = JsonDocument.Parse(content);
        var toolsDirectory = json.RootElement.GetProperty("toolsDirectory").GetString();

        toolsDirectory.ShouldNotBeNull();
        Path.IsPathRooted(toolsDirectory).ShouldBeTrue();
    }

    [Fact]
    public async Task GetStatus_ToolsDirectory_ResolvesRelativeToContentRoot()
    {
        // Arrange
        var client = _fixture.CreateClient();
        ...
        // Default value is "tools" which should resolve to an absolute path ending with "tools"
        toolsDirectory.ShouldNotBeNull();
        toolsDirectory.ShouldEndWith("tools");
    }
