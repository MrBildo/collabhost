...
public class ContractRecordPlacementTests
{
    ...
    // 20 today: 13 from the T4 P2 baseline, plus the two *Command records the #406 spine PR 2 adds
    ...
    // (CreateAppCommand, in Registry/_OperationContracts.cs). The pre-existing CreateAppRequest +
    // CreateAppResponse were already in the baseline count and are unaffected by the migration. PR 5's
    // UpdateSettingsOutcome and PR 6's CreateAppOutcome are *Outcome records, NOT in the *Command/
    // *Request/*Response/*Arguments suffix family, so they are not counted (same as AppActionOutcome /
    // ProxyReloadOutcome). Each later spine PR that adds a public *Command record bumps this count. The
    // count guard keeps the placement assertion honest -- a detector that matches zero would let
    // placement pass vacuously.
    private const int _expectedContractRecordCount = 20;
