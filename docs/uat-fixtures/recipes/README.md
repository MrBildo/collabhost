...
Reproducible build recipes for the UAT fixtures consumed by `docs/release-uat.md` §§ 2-5. One subdirectory per hosted app type:

| Recipe | Variants produced | Source toolchain | Card |
|---|---|---|---|
| [`static-site/`](./static-site/) | `basic/`, `with-config-json/`, `spa-bundle/` | pure data (no build) | #353 |
| [`nodejs-app/`](./nodejs-app/) | `with-start-script/`, `no-start-script/`, `malformed-package-json/` | Node stdlib only at runtime | #353 |
| [`dotnet-app/`](./dotnet-app/) | `framework-dependent/`, `self-contained/`, `self-contained-pdb-stripped/` | .NET SDK 10.x | #353 |
| [`executable/`](./executable/) | `single-binary/`, `multiple-binaries/`, `looks-like-dotnet/` | Go 1.21+ (+ dotnet-app for `looks-like-dotnet`) | #353 |
| [`external-route/`](./external-route/) | `localhost-http/` | pure data; operator launches `python3 -m http.server` from the built dir | #353 |
...
| `dotnet-app` source dir (`*.csproj` at root) | **deferred** — out of scope for v1.5.0 UAT (the runbook walks the publish-output path, not the source-dir path). Re-evaluate when a runbook leg explicitly exercises `DotNetProject` discovery. |
...
| `static-site` has `index.htm` or `default.html` | **deferred** — secondary index variants are not exercised by the v1.5.0 runbook; add when § 4 calls for them. |
