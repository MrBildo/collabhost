...
| `/api/v1/version` matches the release archive AND `wwwrootHash` is intact | (a) `/api/v1/version.version` matches the version string of the archive that was installed. (b) `cat <install-dir>/wwwroot.sha256` matches `/api/v1/version.wwwrootHash`. | (a) Mismatch on `version` means the wrong archive was installed; stop the leg and verify fixtures. (b) Mismatch on `wwwrootHash` means the on-disk `wwwroot/` has drifted from the archive — partial strip, post-install tamper, or a wrong-version `wwwroot/` was copied in. Card #342 silent-failure mode 7's partial-strip variant. |
...
**Per-app `/etc/hosts` precondition (card #345 — closed):** registered apps need `<slug>.<baseDomain>` to resolve from a browser on the Collabhost host before §6 routing assertions can pass against the real domain. Run `sudo collabhost --update-hosts` (Linux/macOS) or `Start-Process collabhost -ArgumentList '--update-hosts' -Verb RunAs` (Windows) after each registration; the CLI writes a managed `# BEGIN COLLABHOST` block to `/etc/hosts` (or `%SystemRoot%\System32\drivers\etc\hosts`) covering the Portal hostname and every registered slug. The `POST /api/v1/apps` 201 response now carries a `helpfulNextSteps` array reminding the operator. See INSTALL.md §9.10.2 for the full alternatives (LAN-side DNS wildcard, real-domain switch).
...
- `POST /api/v1/apps/{slug}/stop` transitions `Running` → `Stopping` → `Stopped`. The operator-stopped flag is set (`StoppedByOperator = true`) — auto-restart does NOT fire after operator stop. Per Card #350, this flag is **persisted** to the `Apps` table — so the stopped state also survives Collabhost restart (auto-start is suppressed on the next boot until the operator issues a Start).
...
- `runtime-config-file` capability serves `/config.json` via a **path-scoped overlay subroute** (#369), placed ahead of the artifact-rooted `file_server`. The overlay is a Caddy subroute matched on the config path (`/config.json` per type default); inside it, a `vars` handler switches the root to the app's **writable data dir** (`<dataDir>/app-data/<slug>/`, resolved by `AppDataPathResolver`), a `headers` handler sets `Cache-Control: no-cache`, and a `file_server` serves the file from that writable root. The path match confines the root switch to the config path so the SPA fallback still stats the artifact root for every other request. The file is **written to the writable data dir, not the artifact dir** — `RuntimeConfigFileWriter.RenderAsync` targets `<dataDir>/app-data/<slug>/config.json`, which lives inside the system-scope unit's `ReadWritePaths` (the #336 artifact-dir target was read-only on a Standard-compliant install — EROFS on the atomic-rename temp sidecar — so #369 moved the write target). **Folded into the per-app settings walk** — when the operator sets non-empty values on the `runtime-config-file` capability, verify the file is materialized at `<dataDir>/app-data/<slug>/config.json` post-save with the resolved values, and `curl -k https://<slug>.collab.internal/config.json` returns it with `Cache-Control: no-cache`.

**Upgrade-path orphan warning (#377 — replays the #375 incident the in-tree tests structurally cannot).** The in-tree integration tests build the orphan end-state directly; they do not replay the operator-upgrades-a-real-install transition that took the Collaboard portal down (#375): a v1.6.2-era install registered runtime-config values while that release's writer was inert, the values persisted, then an upgrade activated the overlay against a file that was never written → the config path 404s and the SPA breaks. Cover the upgrade path here on a real install:
  1. **Build the upgrade fixture.** On an install carrying a static-site app with the `runtime-config-file` capability, register non-empty runtime-config values for the app (via settings save or import) so a `runtime-config-file` override row with `values` is persisted, then ensure the overlay file is **absent** from the writable data dir — delete `<dataDir>/app-data/<slug>/config.json` if present (simulating the pre-#369 install that never wrote it). Leave the app's route enabled (the default post-hydration state).
  2. **Boot Collabhost** and capture the startup log (stderr / journal).
  3. **Assert the boot log carries the orphan warning** at `Warning` level. The line must name (a) the affected app slug, (b) the expected file path `<dataDir>/app-data/<slug>/config.json` (the writable-dir target, NOT the artifact dir), and (c) the remedy — re-apply/save the runtime-config values (or import them) to write the file.
  4. **Confirm it is a soft warning, not a halt** — boot completes, the API comes up, and `/api/v1/status` responds. The check runs from `ApplicationStarted` and is fully guarded; a diagnostic failure must never destabilize boot.
  5. **Negative control:** save the runtime-config values again so the file is written, restart, and assert the warning does NOT recur (the orphan is resolved).
...
- `external-route` has NO `auto-start` capability binding (per Card #348 spec §4) — but its route-state is in-memory only and defaults to enabled on first read. Confirm a previously-running external-route is back to `running` post-Collabhost-restart (route-enabled by default-true semantics, not by auto-start). A previously-stopped external-route **also stays stopped** post-restart: per Card #350, the `Stop` endpoint persists `StoppedByOperator = true` to the `Apps` table and `ProxyManager.HydrateRouteStatesFromPersistenceAsync` reads that column on boot before the first sync, so the disabled route survives the restart. Pre-#350 (v1.4.x and earlier) the in-memory `false` from `DisableRoute` did not persist and the route silently re-enabled — that asymmetry is now closed for all six built-in app types. Record per leg.
...
- **Persisted operator-stop across restart (Card #350).** Stop `dotnet-app`, `nodejs-app`, `static-site`, AND `external-route` (one at a time, via `POST /api/v1/apps/{slug}/stop`), then restart Collabhost. On boot, ALL FOUR must stay stopped — auto-start is suppressed for the process-bearing types via `ProcessSupervisor.StartAsync`'s persisted-flag check, and the route stays disabled for `static-site` and `external-route` via `ProxyManager.HydrateRouteStatesFromPersistenceAsync`. Issue Start (`POST .../start`) on each — they should come back up and stay up across a second restart (the Start clears the persisted flag). A regression here re-introduces the pre-#350 silent re-enable on routing-only types or the latent process-bearing gap. Record per leg.

**Static-site auto-start:** static-site does NOT have an `auto-start` capability binding in the type defaults — but auto-start of a static-site means re-enabling the Caddy route, not starting a process. Confirm a started static-site is restored to `running` (route-enabled) post-Collabhost-restart, and (per the persisted-stop bullet above) that a stopped static-site stays stopped.
...
7. **`proxyState: "running"` AND the SPA shell is unreachable.** Static-files middleware has no `wwwroot/`. Assert SPA shell curl returns `<!DOCTYPE html` (§0). **Partial-strip detection (#342):** the `wwwrootHash` assertion in §0 catches the partial-stripped variant (assets present but stale, or some chunks missing) that the SPA-shell curl alone misses — the archive sidecar `wwwroot.sha256` and the running binary's `/api/v1/version.wwwrootHash` must match.
