...
- 7 built-in app types: `dotnet-app`, `nodejs-app`, `static-site`, `executable`, `internal-service`, `system-service`, `external-route`
...
- `internal-service` (#361) is the inverse of `external-route`: Collabhost runs and supervises the process, but no Caddy route is generated. Targets non-HTTP managed processes (databases, key-value stores like Valkey, message brokers, custom-protocol upstreams) where an auto-generated reverse_proxy route would be a permanent 502. Has `process` + `artifact` + `restart` + `auto-start` + `environment-defaults`; omits `routing`, `port-injection`, `health-check`, and `security-headers` so no TCP port is allocated and no Caddy route is emitted.
...
- `PortalIntegrityCheck` — soft preflight wired in `ApplicationStarted` immediately after `PortalReachabilityCheck` (skipped when reachability is not `Ok`). Computes a deterministic SHA-256 over the on-disk `wwwroot/` tree and compares it to the hash embedded into the binary at archive-build time (via `AssemblyMetadataAttribute` keyed `WwwrootHash`, surfaced via `VersionInfo.WwwrootHash`). Drift → `LogWarning` (never halt — legitimate operator customization is real); empty embedded hash (dev builds, pre-#342 archives) → silent. The hash algorithm mirrors the workflow's bash computation (`.github/workflows/publish.yml` "Compute wwwroot hash" step). Card #342.
- The Portal is **not** a registered app; it does not appear in `/api/v1/apps`, has no `AppType`, and is not seeded via `ProxyAppSeeder`. The `wwwroot/` directory is part of the archive contract (item 7 of 8), paired with the build-time `wwwroot.sha256` sidecar (item 8 of 8) that lets UAT and `PortalIntegrityCheck` detect drift between shipped and on-disk Portal assets (card #342). `/api/v1/routes` synthesizes a Portal row at index 0 (`isPortal: true`) so operators can see the resolved hostname; `/api/v1/status` carries `portalUrl`; `/api/v1/version` carries `wwwrootHash` for the UAT comparison.
